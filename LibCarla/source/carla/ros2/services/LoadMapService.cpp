...
LoadMapService::LoadMapService(carla::rpc::RpcServerInterface &carla_server,
...
carla_msgs::srv::LoadMap_Response LoadMapService::LoadMap(carla_msgs::srv::LoadMap_Request const &request) {
  ...
  if (request.force_reload() ||
    ...
    auto call_response = _carla_server.call_load_new_episode(map_name_without_prefix, request.reset_episode_settings(),
                                                             static_cast<rpc::MapLayer>(request.map_layers()));
  ...
  if (response.success()) {
    log_warning("ROS2:LoadMapService(", request.mapname(), "): request to load new episode '", map_name_without_prefix,
                "' with force: ", request.force_reload() ? "True" : "False",
                ", reset_episode_settings: ", request.reset_episode_settings() ? "True" : "False",
                " and map_layers: ", request.map_layers(), " succeeded");
  } else {
    log_error("ROS2:LoadMapService(", request.mapname(), "): request to load new episode '", map_name_without_prefix,
              "' with force: ", request.force_reload() ? "True" : "False",
              ", reset_episode_settings: ", request.reset_episode_settings() ? "True" : "False",
              " and map_layers: ", request.map_layers(), " failed: ", error_reason);
  }
  return response;
}
