...
CarlaControlSubscriber::CarlaControlSubscriber(ROS2NameRecord &parent, carla::rpc::RpcServerInterface &carla_server)
  : SubscriberBaseSynchronizationClient(parent, carla_server),
    _impl(std::make_shared<CarlaControlSubscriberImpl>(*this)) {}
...
void CarlaControlSubscriber::ProcessMessages() {
  while (_impl->HasPublishersConnected() && _impl->HasNewMessage()) {
    ...
    carla::log_debug("CarlaControlSubscriber[", ThisAsSynchronizationClient(), "]::ProcessMessages(",
                     carla_control_msg_entry.publisher, ", ", synchronization_participant,
