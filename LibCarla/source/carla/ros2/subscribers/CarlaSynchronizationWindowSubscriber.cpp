...
void CarlaSynchronizationWindowSubscriber::ProcessMessages() {
  while (_impl->HasPublishersConnected() && _impl->HasNewMessage()) {
    ...
    auto const synchronization_participant =
        GetSynchronizationParticipant(carla_synchronization_window_msg_entry.publisher);

    carla::log_debug("CarlaSynchronizationWindowSubscriber[", ThisAsSynchronizationClient(), "]::ProcessMessages(",
                     carla_synchronization_window_msg_entry.publisher, ", ", synchronization_participant,
                     ")=", carla_synchronization_target_game_time);
    _carla_server.call_update_synchronization_window(ThisAsSynchronizationClient(), synchronization_participant,
                                                     carla_synchronization_target_game_time);
  }
}
