...
public:
  explicit SubscriberBaseSynchronizationClient(ROS2NameRecord &parent, carla::rpc::RpcServerInterface &carla_server)
    : SubscriberBase<MESSAGE_TYPE>(parent), _carla_server(carla_server) {}
  ...
  virtual ~SubscriberBaseSynchronizationClient() {
    if (!_synchronization_client_id.empty()) {
      for (auto [publisher_guid, participant] : _carla_synchronization_window_participants) {
        carla::log_debug("~SubscriberBaseSynchronizationClient[", _synchronization_client_id,
                         "]:: disconnect publisher (", publisher_guid, ", ", participant, ")");
  ...
  void PublisherConnected(std::string const &publisher_guid) override {
    ...
    auto carla_synchronization_window_participant =
        _carla_server.call_register_synchronization_participant(ThisAsSynchronizationClient()).Get();
...
protected:
  ...
  carla::rpc::synchronization_participant_id_type GetSynchronizationParticipant(std::string const &participant_guid) {
    ...
    carla::log_error("SubscriberBaseSynchronizationClient[", ThisAsSynchronizationClient(),
                     "]::GetSynchronizationParticipant(", participant_guid, ") participant not found.");
...
private:
  std::map<std::string, carla::rpc::synchronization_participant_id_type> _carla_synchronization_window_participants;
