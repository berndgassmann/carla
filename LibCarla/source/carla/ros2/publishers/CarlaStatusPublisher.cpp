...
bool CarlaStatusPublisher::Init(std::shared_ptr<DdsDomainParticipantImpl> domain_participant) {
  // provide the status transient local to ensure if CARLA is stuck and someone wants to query the synchronization
  // status, then the last published state is still available and one is able to detect for what CARLA is waiting
