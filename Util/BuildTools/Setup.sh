...
set(CMAKE_CXX_FLAGS_RELEASE_CLIENT "\${CMAKE_CXX_FLAGS_RELEASE} -DNDEBUG -g -fwrapv -O2 -Wall -Wstrict-prototypes -fno-strict-aliasing -Wdate-time -D_FORTIFY_SOURCE=2 -g -fstack-protector-strong -Wformat -Werror=format-security ${CARLA_CFLAGS} ${CARLA_CXXFLAGS} -Wno-missing-braces" CACHE STRING "" FORCE)
EOL
...
set(CMAKE_CXX_FLAGS "\${CMAKE_CXX_FLAGS} ${CARLA_SERVER_CXXFLAGS}" CACHE STRING "" FORCE)
...
for PY_VERSION in ${PY_VERSION_LIST[@]} ; do
  ...
  if { ${SHOULD_BUILD_BOOST} ; } ; then
    ...
    ./b2 toolset="${BOOST_TOOLSET}" cxxflags="${BOOST_CXXFLAGS} ${CARLA_SERVER_CXXFLAGS}" linkflags="${CARLA_SERVER_LINKER_FLAGS}" --with-system --prefix="../${BOOST_BASENAME}-server-install" -j ${CARLA_BUILD_CONCURRENCY} stage release install
    popd >/dev/null
    ...
    ./b2 toolset="${BOOST_TOOLSET}" cxxflags="${BOOST_CXXFLAGS}" --prefix="../${BOOST_BASENAME}-client-install" -j ${CARLA_BUILD_CONCURRENCY} stage release install
...
# ==============================================================================
# -- Get rpclib and compile it for server and client ------
# ==============================================================================
...
if [[ -d "${RPCLIB_BASENAME}-server-install" && -d "${RPCLIB_BASENAME}-client-install" ]] ; then
  ...
  log "Building rpclib for server."
  ...
  log "Building rpclib for client."
...
# ==============================================================================
# -- Get GTest and compile it for server --------------------------------------
# ==============================================================================
...
if [[ -d "${GTEST_BASENAME}-server-install" && -d "${GTEST_BASENAME}-client-install" ]] ; then
  ...
  log "Building Google Test for server."
  ...
  log "Building Google Test for client."
...
# ==============================================================================
# -- Get Recast&Detour and compile it for client ------------------------------
# ==============================================================================
...
if [[ -d "${RECAST_BASENAME}-client-install" &&
  ...
  log "Building Recast & Detour for client."
...
if [[ -d ${XERCESC_BASENAME}-client-install &&  -d ${XERCESC_BASENAME}-server-install ]] ; then
  ...
  log "Building xerces-c for server."
  ...
  log "Building xerces-c for client."
...
if ${USE_CHRONO} ; then
  ...
  # ==============================================================================
  # -- Get Chrono and compile it for server -------------------------------------
  # ==============================================================================
...
PATCHELF_INSTALL_CLIENT_DIR=patchelf-client-install
...
if ${USE_PYTORCH} ; then
  ...
  LIBTORCH_PATH=${PWD}/${LIBTORCH_BASENAME}-server-install
  ...
  LIBTORCHSCATTER_INSTALL_SERVER_DIR=${PWD}/${LIBTORCHSCATTER_BASENAME}-server-install
  LIBTORCHSCATTER_INCLUDE=${LIBTORCHSCATTER_INSTALL_SERVER_DIR}/include
  LIBTORCHSCATTER_LIB=${LIBTORCHSCATTER_INSTALL_SERVER_DIR}/lib
  LIBTORCHSCATTER_REPO="https://github.com/rusty1s/pytorch_scatter.git"

  build_torch_extension ${LIBTORCHSCATTER_SRC_DIR} ${LIBTORCHSCATTER_INSTALL_SERVER_DIR} "${LIBTORCHSCATTER_REPO}"
  ...
  LIBTORCHCLUSTER_INSTALL_SERVER_DIR=${PWD}/${LIBTORCHCLUSTER_BASENAME}-server-install
  LIBTORCHCLUSTER_INCLUDE=${LIBTORCHCLUSTER_INSTALL_SERVER_DIR}/include
  LIBTORCHCLUSTER_LIB=${LIBTORCHCLUSTER_INSTALL_SERVER_DIR}/lib
  LIBTORCHCLUSTER_REPO="https://github.com/rusty1s/pytorch_cluster.git"

  build_torch_extension ${LIBTORCHCLUSTER_SRC_DIR} ${LIBTORCHCLUSTER_INSTALL_SERVER_DIR} "${LIBTORCHCLUSTER_REPO}"
...
if ${USE_ROS2} ; then
  if [[ -d ${FASTDDS_BASENAME}-server-install ]] ; then
    ...
    # since ASIO is header only installation, we don't care about flags from CARLA_SERVER_TOOLCHAIN_FILE
...
elseif (CMAKE_BUILD_TYPE STREQUAL "Pytorch")
  ...
  list(APPEND CMAKE_PREFIX_PATH "${LIBTORCHSCATTER_INSTALL_SERVER_DIR}")
  list(APPEND CMAKE_PREFIX_PATH "${LIBTORCHCLUSTER_INSTALL_SERVER_DIR}")
